...
We test our approach on object manipulation and autonomous driving examples. The algorithm performance is compared against a state-of-the-art TAMP planner in the fully observable case.
...
\begin{itemize}
\item \textbf{Look} a block: the robot seeks to align its sensor (assumed to be the robot head) with the colored side of the box. This will typically lead the robot to both move its head and its arm simultaneously (see Fig.~\ref{fig:look-1}). After this action, the robot receives an observation (color of the block).
...
To evaluate the scalability, we test with three different initial belief states configurations. In the first configuration (A), the agent has a prior knowledge of the color of each block. This boils down to the fully observable case. In the second configuration (B), 2 blocks are unknown. Fig.~\ref{fig:start-1} and ~\ref{fig:start-2} show the two possible start configurations with this model. In the third case (C), there are no prior knowledge which leads to 6 possible initial configurations. In all cases, the initial belief state is uniform (e.g. 1/2 likelihood for each possible start configuration with the model B, 1/6 likelihood for model C).
...
\hline
A1                    & 1 & \thead{3/3} &\thead{yes} &  192 & 0.20 \\
\hline
A2                    & 1 & \thead{3/3} &\thead{no} &  192 & 0.24 \\
\hline
B1                    & 2 & \thead{1/3} &\thead{yes} &  336 & 0.53 \\
\hline
B2                    & 2 & \thead{1/3} &\thead{no} &  480 & 1.08 \\
\hline
C1                    & 6 & \thead{0/3} &\thead{yes} &  2076 & 8.55 \\
\hline
C2                    & 6 & \thead{0/3} & \thead{no} &  4128 & 34.3 \\
\hline
\end{tabular}
...
The overall planning time is dominated by the motion planning (see table.~\ref{table:times} As long as the model is simple (A or B) or the exploration kept low ($C_0$ = 0.25), motion planning is dominated by the single pass of joint optimization. The execution time of this pass mainly depends on the total number of action steps in the policy and on the belief state size but is independent from the number of iterations occurring before. In the configurations requiring the biggest number of iterations (C1 and C2 with $C_0$ = 0.015), motion planning and the overall planning time are dominated by the piecewise motion planning phases of the policy improvement iterations. %It also appears clearly that scalability is a crucial problematic here. All the parameters of the problem increase drastically with the size of the belief state (graph size, required number of iterations, size of the resulting policy, computation time).%
...
We used the block example introduced above to benchmark the proposed solver against the \textit{TMKit framework} in the fully-observable case. This solver implements the \textit{Incremental Task and Motion Planning} algorithm (IDTMP) (see [10]). We added an 4-th block in the example to further test the scalability. The benchmark was conducted on an 
Intel\textregistered Core\texttrademark i3-4100M CPU under Ubuntu 16.04.
\begin{center}
\footnotesize
...
\bibitem{c10} N. T. Dantam, Z. K. Kingston, S. Chaudhuri, and L. E. Kavraki, Incremental Task and Motion Planning: A Constraint-Based Approach, in Robotics: Science and Systems, 2016

...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\marc{use bibtex!!! Oh oh}% I change to bibtex as soon as I find time
...
After having taken an action $a$ and received an observation $o$, the relation between the current belief state $b$ and the new  $b'$ obeys the same equation than a POMDP. 

\begin{align*}
b'(s') = \alpha O(o | s', a)\sum_{s} T(s'|s, a)b(s), \forall s \in S, \forall s' \in S
\end{align*}

In the above equation, $\alpha$ is a normalization constant.
%For a belief state $b$. We note $b(s)$ the probability of being in state $s$.
...
A TAMP policy is the couple formed by a policy $\pi$ and its trajectory tree $\psi$. Its cost is given by,

\begin{equation}
c(\pi, \psi) = \sum_{b\in\pi}p(b | \pi, b_0) c(\pi(b), \psi(b))~.
\end{equation}

...
=======
We consider the Baxter and three blocks on a table (see Fig.~\ref{fig:block-world} ). The robot has to stack the blocks in a given color order (blue, green, and red on the top). The blocks just have one side colored (assumed to be the opposite side). The robot knows where the blocks are (referred as $block\_1$, $block\_2$, $block\_3$). However it cannot see the colored side from behind and has explore to identify the blocks and to build the stack in the correct order.
>>>>>>> experiment_rewriting
\begin{figure}
\centering
...
=======
\item \textbf{Look} a block: the robot aligns its head with the colored side of the block. This typically leads the robot to both move its head and its arm simultaneously (see Fig.~\ref{fig:look-1}). An observation is received after this action(block's color).
>>>>>>> experiment_rewriting
...
\hline
\multirow{3}{*}{B}     & 0.25  & 5 & 12 & 0.10 & 8.83 & 14.1 & 23.1 \\
				        & 0.1   & 5 & 12 & 0.10 & 8.02 & 15.1 & 23.3 \\
					    & 0.015 & 7 & 12 & 0.17 & 10.7 & 14.1 & 25.1 \\
					    
\hline
\multirow{3}{*}{C}     & 0.25  & 11 & 33 & 0.81 & 42.6 & 66.9 & 111.0 \\
				        & 0.1   & 22 & 33 & 1.69 & 61.8 & 66.9 & 131.1 \\
					    & 0.015 & 22 & 33 & 1.77 & 66.3 & 67.2 & 135.2 \\
\hline
\multirow{3}{*}{D}     & 0.25  & 80 & 33 & 10.2 & 97.2 & 84.9 & 193.6 \\
...
\begin{tikzpicture}
\begin{axis}[
    title={Values of best feasible policy for B1},
   ...
   % dashed,
   % ]
   % table [col sep=comma]{data/C1/0.25/policy-candidates.data};
   % \addlegendentry{$candidate, C_0=0.25$}
    
%\addplot[ 
%    color=purple,
%   ]
%    table [col sep=comma]{data/C1/0.25/policy-results.data};
...
%    dashed,
%    ]
%    table [col sep=comma]{data/C1/0.1/policy-candidates.data};
%    \addlegendentry{$candidate, C_0=0.1$}
    
%\addplot[ 
%    color=blue,
%    ]
%    table [col sep=comma]{data/C1/0.1/policy-results-max.data};
...
%    dashed,
%    ]
%    table [col sep=comma]{data/C1/0.015/policy-candidates.data};
%    \addlegendentry{$candidate, C_0=0.015$}
    
%\addplot[ 
%    color=orange,
%    ]
%    table [col sep=comma]{data/C1/0.015/policy-results-max.data};
%    \addlegendentry{$result, C_0=0.015$}
             
\addplot [color=red, only marks] table {10 -2.58801
}node[above,pos=1] {$C_0=0.25$};
\addplot [color=red, only marks] table {21 -2.60166
}node[above,pos=1] {$C_0=0.1$};
\addplot [color=red, only marks] table {27 -1.77868
}node[above,pos=1] {$C_0=0.015$};
...
\subsubsection{Influence of $C_0$ initialization}
